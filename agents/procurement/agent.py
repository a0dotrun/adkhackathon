...
def confirm_and_exit_loop_tool(tool_context: ToolContext):
    """
    Call this function ONLY when the user has explicitly confirmed that all
    extracted line item data (SKU, Quantity, Rate, Amount) is correct.
    This signals that the iterative data extraction and confirmation process should end.
    """
    print(f"  [Tool Call] confirm_and_exit_loop_tool triggered by {tool_context.agent_name}")
    tool_context.actions.escalate = True  # This signals the LoopAgent to terminate
    # This tool also implies that the final JSON should be generated by the preceding agent.
    # Let's also set a flag that the data is officially confirmed.
    tool_context.state["line_item_data_status"] = "officially_confirmed_final"
    return {"status": "Loop termination signaled due to user confirmation."}
